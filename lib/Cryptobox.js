...
class Cryptobox {
  ...
  constructor(options = defaults) {
    ...
    this.checkAndInitMinRequirement(options);
   ...
   * @returns { void } void
   */
  checkAndInitMinRequirement(options) {
    let value = '';
    for (let key in options) {
      value = options[key];
      this.minRequirements[key]
   ...
   * @param { defaults } options - A set of options passed by the user
   * @return { void } void
   */
  setCoinInfo(options) {
   ...
   */
  checkKeyPattern(key) {
    const patternMatch = '/[^A-Za-z0-9]/'.search(key);
